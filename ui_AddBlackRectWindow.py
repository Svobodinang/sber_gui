...
class ui_AddBlackRectWindow(QtWidgets.QDialog):
        ...
        if os.path.exists(self.pathCamera):
            ...
            if os.path.getsize(self.pathCamera) > 0:
                ...
                with open(self.pathCamera, "r") as read_file:
                    ...
                    if 'rects_black' in data:
                        for rect in data['rects_black']:
                            rect_item = MyRect(QtCore.QRectF(rect[0] * 0.4, rect[1] * 0.4,
                                                             rect[2] * 0.4, rect[2] * 0.4))
                            # Добавляем на сцену
                            self.scene.addItem(rect_item)
                            # print(rect_item.sceneBoundingRect().x())
                            # Добавляем в глобальный массив
                            Global.arrBlackRects[int(Global.numCamera)].append(rect_item)
    ...
    def deleteLast(self):
        ...
        if len(Global.arrBlackRects[numCamera]) > 0:
            # Удаляем со сцены(последний)
            self.scene.removeItem(Global.arrBlackRects[numCamera][len(Global.arrBlackRects[numCamera]) - 1])
            # Удаляем из массива(последний)
            del Global.arrBlackRects[numCamera][len(Global.arrBlackRects[numCamera]) - 1]
    ...
    def addRectsToData(self, data):
        ...
        for rect in Global.arrBlackRects[int(Global.numCamera)]:
            ...
            data['rects_black'].append(arrRectRoi)
        return data
